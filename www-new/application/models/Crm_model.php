...
class Crm_model extends CI_Model {
    ...
    public function login($u_email, $password){
        ...
        if($query->num_rows()==1){
            ...
            if($query->num_rows()==1){
                ...
                $this->session->set_userdata($data);
                redirect(base_url().'Crm/client1', 'refresh');
                return TRUE;
            }
            else{
                return FALSE;
    ...
    public function no_of_client(){
       ...
       $this->db->select("COUNT(crm.id) AS no_of_clients");
       $this->db->from('crm');
